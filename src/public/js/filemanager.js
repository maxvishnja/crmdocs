...
$(document).ready(function(){
        ...
        $.ajax({
            ...
            if(data.error){
                ...
                $.each(data.data, function(index, value){
                    $('.permis').append("<li class='group fa fa-user' id='" + value.id + "'> " + value.name + "</li>");
                });
        ...
        if(!checkFileSelected()){
            var permissions = {
                ...
                callback: function (key, options) {
                    //new PNotify({
                    //    title: 'Success',
                    //    text: 'Permission succesfully',
                    //    type: 'info'
                    //});
                    $('.permis').html('');

                    getGroups();
                    $('#modalPerm').modal('show');
                }
    ...
    $('.permopen').on('click',function(){
            $('.permis').slideToggle(500);
        }
    );
